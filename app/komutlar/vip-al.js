...
exports.run = async(client, message, args) => {
   if(!message.member.roles.cache.has('785617874227822604')) return message.channel.send('Bu komutu kullanabilmek için gerekli yetkiye sahip değilsin : `rôl adı`')
   let member = message.mentions.users.first() || client.users.cache.get(args.join(' '))
   if(!member) {
       return message.channel.send('Bir kişi etiketlemelisin')
   }
   let vip = message.guild.roles.cache.find(r => r.id === '785742427126366229')//Viprolİd Koy
  ...
  client.channels.cache.get('785769660388868119').send(embed)///LOG KANAL İD YAZMALISIN
}
