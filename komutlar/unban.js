...
exports.run = async (client, message, args) => {
  ...
  if (!message.member.hasPermission("785617880049254411")) return message.channel.send(`**Bu komutu kullanabilmek için "\`Üyeleri Yasakla\`" yetkisine sahip olmalısın.**`);
 ...
 if (isNaN(user)) return message.channel.send('**Lütfen Banını Açmak İstediğiniz Üyeninin ID sini Girin**');
  ...
  client.channels.cache.get('785749427474137089').send(embed)///LOG KANAL İD YAZMALISIN
